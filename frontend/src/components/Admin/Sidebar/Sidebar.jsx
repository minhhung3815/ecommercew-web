            ...
            <div className="flex flex-col w-full gap-0 my-8">
                {navMenu.map((item, index) => {
                    ...
                    return (
                        <>
                            {label === "Logout" ? (
                                <button onClick={handleLogout} key={item.id} className="hover:bg-gray-700 flex gap-3 items-center py-3 px-4 font-medium">
                                    <span>{icon}</span>
                                    <span>{label}</span>
                                </button>
                            ) : (
                                <Link to={ref} key={item.id} className={`${activeTab === index ? "bg-gray-700" : "hover:bg-gray-700"} flex gap-3 items-center py-3 px-4 font-medium`}>
                                    <span>{icon}</span>
                                    <span>{label}</span>
                                </Link>
                            )}
